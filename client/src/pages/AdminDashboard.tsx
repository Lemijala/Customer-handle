...
const AdminDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-slate-900 dark:text-white">
              ...
              <>
                ...
                <div className="absolute right-0 top-12 w-80 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl z-50 overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-700">
                    ...
                    <button onClick={() => setShowNotifications(false)} className="text-xs text-blue-500 hover:underline">Close</button>
                  </div>
                  <div className="max-h-72 overflow-y-auto">
                    {/* Unread contacts — clickable, navigates to contacts tab */}
                    {contacts.filter(c => !c.isRead).map(c => (
                      <button
                        key={c._id}
                        className="w-full flex items-start gap-3 px-4 py-3 border-b border-gray-50 dark:border-gray-700/50 hover:bg-blue-50 dark:hover:bg-blue-500/10 transition-colors text-left bg-blue-50/50 dark:bg-blue-500/5"
                        onClick={() => {
                          markRead(c._id);
                    ...
                    {/* Subscribers — clickable, navigates to subscribers tab */}
                    {subscribers.slice(0, 3).map(s => (
                      <button
                        key={s._id}
                        className="w-full flex items-start gap-3 px-4 py-3 border-b border-gray-50 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors text-left"
                        onClick={() => { setActiveTab('subscribers'); setShowNotifications(false); }}
                      >
                    ...
                    {contacts.filter(c => !c.isRead).length === 0 && subscribers.length === 0 && (
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 pt-24 flex flex-col gap-8">
